...
const Products = () => {
  ...
  const handleSaveProduct = async (productToSave: any) => {
    ...
    const endpoint = isEditing
      ? `${API_URL}/products/update/${productToSave.id}`
  ...
  // === Safe Filter (fix for "toLowerCase" error) ===
  ...
  if (isLoading)
    return (
      <div className="p-6 text-center text-gray-500">
        Authenticating and loading product data...
      </div>
    );
  if (error)
    return (
...
// === Dummy placeholders for modals to avoid compile errors ===
