...
const Billing = () => {
  ...
  return (
    <div className="flex h-full gap-6 p-6 bg-gray-100">
      {/* --- Main Billing Area --- */}
      <div className="flex-1 flex flex-col gap-4 print-area">
                  ...
                  <tr key={item.id} className="border-b">
                    ...
                    <td className="p-2">
                      <div className="flex items-center gap-2">
                        <button onClick={() => updateQuantity(item.id, item.billQty - 1)} className="p-1 rounded-full bg-gray-200 hover:bg-gray-300 no-print"><Minus size={14}/></button>
                        <span>{item.billQty}</span>
                        <button onClick={() => updateQuantity(item.id, item.billQty + 1)} className="p-1 rounded-full bg-gray-200 hover:bg-gray-300 no-print"><Plus size={14}/></button>
                      </div>
                    </td>
                    <td className="p-2">₹{item.price.toFixed(2)}</td>
                    <td className="p-2">
                      <input type="number" value={item.discount} onChange={e => updateDiscount(item.id, parseFloat(e.target.value) || 0)} className="form-input w-20 text-right no-print"/>
                    </td>
      ...
      <aside className="w-80 flex-shrink-0 flex flex-col gap-4 no-print">
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm space-y-3">
          ...
          <ActionButton icon={Printer} label="Print" shortcut="F3" onClick={() => window.print()} />
                ...
                <tbody>
                  {foundBill.items.map(item => (
                    <tr key={item.id} className="border-t">
                      <td className="p-2 font-medium">{item.name}</td>
                      <td className="p-2">{item.billQty}</td>
